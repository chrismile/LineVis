...
def replay():
    ...
    start_yaw = -1.5708
    start_pitch = 0.0
    start_fov = g.get_default_camera_fov_deg()
    ...
    def update_fov(t):
        fov = start_fov + (stop_fov - start_fov) * t
        g.set_camera_fovy_deg(fov)

    update_discrete(move_function=update_fov, steps=4, time_per_step=0.5)

    g.set_duration(5.0)

    g.set_duration(0)
    g.set_camera_fovy_deg(g.get_default_camera_fov_deg())
    g.set_rendering_algorithm_settings({
        'reset_accum_every_frame': False,
    })
    g.set_duration(0)
